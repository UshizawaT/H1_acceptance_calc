...
void drawHist(){
  ...
  TH1D *hist_edeps1 = new TH1D("edep1","edep1", 20, 0, 1000);
  TH1D *hist_edeps2 = new TH1D("edep2","edep2", 20, 0, 1000);
  int n = vol_ids.size();
  for(int i=0; i<n; ++i){
  ...
  tc->cd(2);
  hist_edeps1->Draw();
  hist_edeps2->SetLineColor(1);
  hist_edeps2->Draw("same");
}

void analysis_simu(int angle, int sigma, int endseed, TString basename){
  loadFiles(angle, sigma, endseed, basename);
  drawHist();
}
